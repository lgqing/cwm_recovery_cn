...
int use_migrated_storage() {
	char get_version[PROPERTY_VALUE_MAX];
    property_get("ro.android.version", get_version, "");
	return !(strncmp(get_version,"4.2",3));
}
...
void write_recovery_version() {
    if (is_data_media()) {
        ...
        else write_string_to_file("/data/media/clockworkmod/.recovery_version",EXPAND(RECOVERY_VERSION) "\n" EXPAND(TARGET_DEVICE));
    } else {
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            case 4:
                ...
                break;
            case 5:
				if (confirm_selection( "Confirm fix?", "Yes - Fix permissions")) {
